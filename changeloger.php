...
?>
	...
	<ul class="<?php echo $is_child ? "changeloger-version-list-wrapper" : ""  ?>">
		<?php foreach ($versions as $version) : ?>
			...
			<?php if ((isset($version['children']) && count($version['children']) > 0 && changelogger_can_show_children($available_versions, $version['children'])) || in_array($version['version'], $available_versions, true)) : ?>
...
function changeloger_render($attributes, $content, $instance)
{
	...
	$pagination_type = isset($attributes['paginationType'])
	...
	if (true === $enable_pagination) {
		if ('numbered' === $pagination_type) {
			$versioned_changelog =
				array_slice($changelog, $start_index, $per_page);
		} else {
	...
	$load_more_button_markup = 'load-more' === $attributes['paginationType'] ? '
		<div class="wp-block-button">
			<a style="'. changloger_convert_array_to_style_declaration($pagination_styles) .'" class="changeloger-pagination-button wp-block-button__link wp-element-button">'.$pagination_text.'</a>
		</div>
	' : "";
...
?>
	<div <?php echo $wrapper_attributes ?> data-total-page=" <?php echo $total_page ?>" data-per-page="<?php echo $per_page ?>" data-enable-pagination="<?php echo $attributes['enablePagination'] ? "true" : "false" ?>" data-pagination-type="<?php echo $pagination_type ?>">
					...
					<div id="<?php echo $item['version'] ?>" class="changelog-info-item">
						...
						<div class="content">
							<?php foreach ($item['changes'] as $change) : ?>
								<p>
									<span class="<?php echo 'tag ' . str_replace(" ", "-", strtolower($change['category']))  ?>" <?php echo changeloger_get_custom_style($attributes, strtolower($change['category'])) ?>>
